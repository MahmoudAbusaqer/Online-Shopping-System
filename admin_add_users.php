...
$num1=0;
$result3= $con->query("SELECT * FROM `admin-customer` where `is_admin` = 'no'");
...
<div class="div3 container ">
    <form name="userForm" method="post">
        <div class="form-group">
            ...
            $password_selected='';
            $dis=false;
            $result1= $con->query("SELECT * FROM `admin-customer` where `is_admin` = 'no'");
            for ($i=0; $i<=$num1; $i++){
                if (isset($_POST[$i])) {
                    $result1->data_seek($i);
                    $row = $result1->fetch_row();
                    $id_selected=$row[0];
                    $password_selected=$row[1];
                    $dis=true;
                }}
            ?>
            <input type="text" class="form-control text" placeholder="ID" name="id" value="<?php echo $id_selected;?>" >
            <input type="password" class="form-control text" placeholder="Password " name="password" value="<?php echo $password_selected;?>">
        </div>
    ...
    <div class="row">
        <table class="table table-striped table-dark">
            <form method="post">
                ...
                <?php
                $num=0;
                $result= $con->query("SELECT * FROM `admin-customer` where `is_admin` = 'no'");
                ...
                if(isset($_POST['insert'])){
                    ...
                    if($id==''|| $found==true){
                        ...
                        $insert= $con->query("insert into `admin-customer` values('".$id."', '".$password."', 'no');");
                        echo "<script> alert('The operation Done Successfully'); location.assign('admin_add_users.php');</script>";
                    }
                }
                ...
                if(isset($_POST['update'])){
                    ...
                    if($found==true){
                        $update=$con->query("update `admin-customer` set name='".$password."' where id =".$id.";");
                        echo "<script> alert('The operation Done Successfully'); location.assign('admin_add_users.php');</script>";
                    }else{
                        echo "<script> alert('The operation Did Not Done Successfully'); </script>";
                    }
                ...
                if(isset($_POST['delete'])){
                    ...
                    if($found==true){
                        $con->query("DELETE FROM `admin-customer` where id= '".$id."';");
                        echo "<script> alert('The operation Done Successfully'); location.assign('admin_add_users.php');</script>";
                     }else{
                        echo "<script> alert('The operation Did Not Done Successfully'); </script>";
                    }
